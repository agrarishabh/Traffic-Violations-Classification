...
# Compatibility shim — remove red param if callers pass it
...
with st.sidebar:
    ...
    for name, desc in [("Home","System overview"),("Live Analysis","Detect violations"),
                        ("Violation Records","Search records"),
                        ("Analytics","Charts & trends"),("Reports","Export data")]:
        st.markdown(f"""
        <div style="padding:9px 14px;border-radius:8px;margin:3px 0;
                    border:1px solid rgba(219,226,220,0.08);cursor:pointer;
                    background:#020202">
            <div style="font-size:0.85rem;font-weight:500;color:#9e9e9e">{name}</div>
            <div style="font-size:0.7rem;color:#424242;margin-top:1px">{desc}</div>
        </div>""", unsafe_allow_html=True)
